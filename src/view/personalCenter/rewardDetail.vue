<template>
  <div class="attention_warp">
    <p class="item_h_title">{{ $t(`Module['打赏明细']`) }}</p>
...
<script>
import { chatGiftsOrderDetail } from '@/api/payment'
export default {
  name: '',
  data() {
    return {
  ...
  methods: {
    ...
    async chatGiftsOrderDetail() {
      const { page, limit } = this.page
      const { Data, Code } = await chatGiftsOrderDetail({ page, limit })
      if (Code === 200) {
