<template>
  <div class="attention_warp">
    <p class="item_h_title">{{ $t(`Module['我的粉丝']`) }}</p>
      ...
      <ul class="attention_list">
        <li class="attention_item" v-for="(item, index) in list" :key="index">
          ...
          <div class="attention_btn">
            <v-btn depressed rounded class="btn3 btn" min-width="88" height="34" @click="goToChat(item)">{{
              $t(`Module['发送消息']`) }}</v-btn>
            <!-- <v-btn depressed rounded class="btn1 btn" v-if="item.room_status === 1" min-width="88" height="34"
              @click="handleLive(item)">{{ $t(`Module['观看直播']`) }}</v-btn>
            <v-btn depressed rounded class="btn3 btn" v-else min-width="88" height="34"
              @click="handleMatch(item)">{{ $t(`Module['查看赛程']`) }}</v-btn>
            <v-btn depressed rounded outlined color="#F9B31F" class="btn2 btn" min-width="88" height="34"
              @click="focusAnchor(item)">{{ $t(`Module['取消关注']`) }}</v-btn> -->
          </div>
        </li>
      </ul>
    ...
    <template v-else>
      <div class="w-full h-100 flex items-center justify-center">
        ...
        <page-empty v-else type="user" :title="$t(`Module['暂无粉丝']`)"></page-empty>
      </div>
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    goToChat(item) {
      ...
      let chatAddressList = []
      const id = this.userInfo.user.id
      const key = 'chatList' + id
      if (storageRead(key)) {
        chatAddressList = JSON.parse(storageRead(key))
      }
      // 不包含则插入
      let isInclude = false
      chatAddressList.forEach(item => {
        if (item.user_id === address.user_id) {
          isInclude = true
        }
      })
      if (!isInclude) {
        chatAddressList = [address].concat(chatAddressList)
      }
      storageSave(key, JSON.stringify(chatAddressList))
    ...
    toSend() {
      ...
      // this.$router.push({ name: 'anchorRoom', params: { id: item.room_id } })
    },
    // handleLive(item) {
    //   if (!this.token) {
    //     this.$bus.$emit('login/on', 'login')
    ...
    async myFollowList() {
      ...
      this.loading = true
      const { Data, Code } = await liveFollowerList({ page, limit })
      if (Code === 200) {
        this.list = Data
      }
      this.loading = false
